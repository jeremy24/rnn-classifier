...
class TextLoader(object ):
	...
	def preprocess_helper(self, seq):
		...
		for char in seq:
			...
			encoded_seq.append(self.vocab[char])
		
		labels = np.array(self.labeler_fn(seq), dtype=np.uint16)
	...
	def preprocess(self, input_file, vocab_file, 
		...
		prepro_data = self.preprocess_helper(seqs)		
		encoded = prepro_data["x"]
		labels = prepro_data["y"]
